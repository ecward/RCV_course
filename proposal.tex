...
%The aim of this project is to integrate some of the components already developed for
...
A detailed explanation of the overall RCV system architecture to be developed in this WP is given in \ref{sec:system_structure_appendix}.
...
\item[Input] None
\item[Output] Well defined, but not functional, ROS interface for sending path following tasks to the RCV and a working ROS interface for accessing the vehicle sensor readings.
\item[Tasks for Integration Milestones]:\
	\begin{description}
		\item[Milestone 1] Well defined and functioning ROS interfaces for the RCV sensor readings. (70h + 70h)\\
  Network Time Protocol (NTP) set up for time synchronization. 10 h\\
  Sensor mounts completed. 15 h 
		\item[Milestone 2] Well defined and functioning ROS interface for issuing trajectory commands to the RCV. (2h + 2h)
...
\item[Summary]
The controller work package will have the main objective of providing a practical trajectory following control functionality. The controller will receive a path in UTM coordinates with a corresponding velocity profile or a time reference for every point of the path. If possible, a control strategy developed for participating in the Grand Cooperative Driving Challenge \cite{gcdc_website} will be reused, otherwise a new controller will be developed. Possible controller alternatives are found in \cite{werling2010tracking} and \cite{lima2015}. In practice, the work will be divided into three parts. Deciding on a control strategy, conditioning of input signals and controller implementation. 
\item[Input]
Feasible trajectory (WP2)
\item[Output] Functioning Path Following Controller
\item[Tasks for Integration Milestones]:\
	\begin{description}
		\item[Milestone 1] Deciding on the control strategy to be used and signal conditioning (20h + 20h)\\
			...
			of the inputs, i.e extracting necessary controller information from available data shall be done. 
		\item[Milestone 2] Implementation of Controller (60h + 60h)\\
			For milestone 2 we aim to have a functioning controller implemented and ready for initial testing 
		\item[Milestone 3] Controller iterated and tuned (30h + 30h)\\
			At this point the controller shall be performing satisfactorily for the proposed demos 
	\end{description}	 
\end{description}
